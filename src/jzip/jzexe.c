...
long search( FILE * f )
{
   ...
   while ( ( c = getc( f ) ) > -1 )
   {
      ...
      else if ( ++i == MAGIC_END )
      {
         ...
         if ( getc( f ) != 0 )
         {
            printf( "Error: standalone flag != 0\n" );
            abort_program( 1 );
         }
         offset = getc( f );
         offset += 256L * getc( f );
   ...
   printf( "Couldn't find magic string." );
   abort_program( 1 );
}
   ...
   {                            
      ...
      else if ( ++magic_idx == MAGIC_END )
      {
         ...
         if ( *magic_pos > 0 )
         {
            ...
            printf( "Found more than one instance of the magic string.\n" );
            fclose( in );
            fclose( out );
            abort_program( 1 );
         }
...
void copy( FILE * in, FILE * out )
{
   ...
   if ( total == 0 )
      printf( "Nothing read from file %s - probably an error.\n", in_name );
}
...
void create_executable( char *story, char *jzip )
{
   ...
   if ( pos == 0 )
   {
      printf( "Error: couldn't find magic string.\n" );
      printf( "Check that you have the right version of JZip.\n" );
      abort_program( 1 );
   }
...
void extract_zcode( char *filename )
{
   ...
   {
      printf( "Unsupported Z code version: %d\n", z_version );
      abort_program( 1 );
   }
   strcpy( fn, filename );
   ext = strrchr( fn, '.' );
