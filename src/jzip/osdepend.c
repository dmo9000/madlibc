...
#define GETOPT_SET    "mvzhy?l:c:k:r:t:" 
   ...
   {                            
      switch ( c )
      {
         ...
         case 'v':             /* version information */
            ...
            printf( "\n\rJZIP - An Infocom Z-code Interpreter Program \n\r" );
            printf( "       %s %s\n\r", JZIPVER, JZIPRELDATE );
            if ( STANDALONE_FLAG )
            {
               printf( "       Standalone game: %s\n\r", argv[0] );
            }
            printf( "---------------------------------------------------------\n\r" );
            printf( "Author          :  %s\n\r", JZIPAUTHOR );
            printf( "Official Webpage: %s\n\r", JZIPURL );
            printf( "IF Archive      : ftp://ftp.gmd.de/if-archive/interpreters/zip/\n\r" );
            printf( "         Based on ZIP 2.0 source code by Mark Howell\n\r\n\r" );
            printf(
                     "Bugs:    Please report bugs and portability bugs to the maintainer." );
            printf( "\n\r\n\rInterpreter:\n\r\n\r" );
            printf( "\tThis interpreter will run all Infocom V1 to V5 and V8 games.\n\r" );
            printf(
                     "\tThis is a Z-machine standard 1.0 interpreter, including support for\n\r" );
            printf( "\tthe Quetzal portable save file format, ISO 8859-1 (Latin-1)\n\r" );
            printf(
   ...
   if ( errflg || optind + expected_args != argc )
   {
      ...
      if ( STANDALONE_FLAG )
         ...
         printf( "usage: %s [options...] story-file\n\r", argv[0] ); 

      printf( "JZIP - an Infocom story file interpreter.\n\r" );
      printf( "      Version %s by %s.\n\r", JZIPVER, JZIPAUTHOR );
      printf( "      Release %s.\n\r", JZIPRELDATE );
      printf( "      Based on ZIP V2.0 source by Mark Howell\n\r" );
      printf( "      Plays types 1-5 and 8 Infocom and Inform games.\n\r\n\r" );
      printf( "\t-l n lines in display\n\r" );
      printf( "\t-c n columns in display\n\r" );
      printf( "\t-r n text right margin (default = %d)\n\r", DEFAULT_RIGHT_MARGIN );
      printf( "\t-t n text top margin (default = %d)\n\r", DEFAULT_TOP_MARGIN );
      printf( "\t-k n set the size of the command history buffer to n bytes\n\r" );
      printf( "\t     (Default is 1024 bytes.  Maximum is 16384 bytes.)\n\r" );
      printf( "\t-y   turn on the legendary \"Tandy\" bit\n\r" ); 
      printf( "\t-v   display version information\n\r" );
      printf( "\t-h   display this usage information\n\r" );

#if defined (HARD_COLORS)
      printf( "\t-f n foreground color\n\r" );
      printf( "\t-b n background color (-1 to ignore bg color (try it on an Eterm))\n\r" );
      printf( "\t     Black=0 Red=1 Green=2 Yellow=3 Blue=4 Magenta=5 Cyan=6 White=7\n\r" );
#endif
      printf( "\t-m   force monochrome mode\n\r" ); 
#if defined __MSDOS__ || defined OS2 
      printf( "\t-b   force black-and-white mode\n\r" ); 
      printf( "\t-o   force color mode\n\r" ); 
      printf( "\t-g   use \"Beyond Zork\" graphics, rather than standard international\n\r" ); 
#elif defined TURBOC            
      printf( "\t-b   run in 43/50 line EGA/VGA mode\n\r" ); 
#endif 

#ifdef STRICTZ
      printf( "\t-s n stricter error checking (default = %d) (0: none; 1: report 1st\n\r",
               STRICTZ_DEFAULT_REPORT_MODE );
      printf( "\t     error; 2: report all errors; 3: exit after any error)\n\r" );
#endif /* STRICTZ */

      printf( "\t-z   display license information.\n\r" );
      exit( EXIT_FAILURE );
   }
